...
def generate_html(articles, result_file="index.html"):
    ...
    for article in articles:
        ...
        for comment in comments:
            comment_html, index = parse_comment(comment, article_url, selected_color="var(--background-color)", index=index)
...
function showRecentComments() {{
  ...
  // 按 timestamp 降序排序（最新在前），再取前 100 条
      ...
      if(currentSortOrder !== "default") {{
         resultsToDisplay.sort(function(a, b) {{
            ...
            return currentSortOrder === "asc" ?
              timeA.localeCompare(timeB) : timeB.localeCompare(timeA);
         }});
      }}
